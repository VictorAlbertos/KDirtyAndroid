...
package presentation.foundation.widgets
...
class Notifications @Inject constructor(private val baseApp: BaseApp) {
    ...
    fun showSnackBar(sTitle: Single<String>) {
        ...
        sTitle.subscribe { title ->
            Snackbar.make(activity.findViewById(R.id.content),
                    title, Snackbar.LENGTH_LONG).apply {
                view.findViewById<TextView>(android.support.design.R.id.snackbar_text)
                        .setTextColor(ContextCompat.getColor(activity, R.color.primary_text_dark))
            }.show()
        }
    }
    ...
    fun showSnackBar(@StringRes idString: Int) {
        ...
        if (activity != null) {
            Snackbar.make(activity.findViewById(R.id.content),
                    baseApp.getString(idString), Snackbar.LENGTH_LONG).apply {
                view.findViewById<TextView>(android.support.design.R.id.snackbar_text)
                        .setTextColor(ContextCompat.getColor(activity, R.color.primary_text_dark))
            }.show()
        }
